 ...
 * @author Amir Paziraee - Smart Mobile Software
...
public class InAppBillingPlugin extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray data, final CallbackContext callbackContext) {
		...
		this.base64EncodedPublicKey="";
		try {

			// Action selector
			/*if ("init".equals(action)) {
                if(data.length() == 0){
                    Log.d(TAG, "You should set app key");
                }else{
                	
                    this.base64EncodedPublicKey=data.getString(0);
                    Log.d(TAG, "Key established: "+this.base64EncodedPublicKey);

                    final List<String> sku = new ArrayList<String>();
                    
                    if(data!=null&&data.getJSONArray(1).length() > 0){
			...
			if ("init".equals(action)) {
                ...
                try {
                    final List<String> sku = new ArrayList<String>();
                    sku.add("Dukan1yBargT1");
                    sku.add("Dukan1ybargT5");
                    sku.add("Dukan1yBargT11");
                    sku.add("Dukan1yBargT20");
                    sku.add("dukan1ybarg");
                    sku.add("dukan1ybargT50");


                    sku.add("Dukan1yBargT19");
                    init(sku);
	...
	private JSONArray getPurchases() throws JSONException {
        ...
        for (Purchase p : purchaseList) {
            JSONObject originalJson = new JSONObject(p.getOriginalJson());
            JSONObject resultJson = new JSONObject();
            resultJson.put("payload", originalJson);
            resultJson.put("signature", p.getSignature());

	        jsonPurchaseList.put(resultJson);
        }
